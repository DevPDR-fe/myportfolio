...
import { Terminal, Cpu, Award, Fingerprint, Wifi, MapPin } from 'lucide-react';

const About: React.FC = () => {
  const [isVisible, setIsVisible] = useState(false);
  const [coords, setCoords] = useState({ lat: -8.0476, lng: -34.8770 });
  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => {
        setCoords(prev => ({
            lat: -8.0476 + (Math.random() * 0.001 - 0.0005),
            lng: -34.8770 + (Math.random() * 0.001 - 0.0005)
        }));
    }, 500);
  ...
  const skills = [
    ...
    "Python", "Solidity", "C++", "Go", 
    "SQL", "Docker", "AWS", "Linux Kernel"
  ];
  ...
  return (
    <section ref={sectionRef} className="py-24 bg-zinc-950 relative overflow-hidden">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        ...
        {/* Section Header */}
        <div className={`mb-12 transition-all duration-700 ${isVisible ? 'opacity-100 translate-x-0' : 'opacity-0 -translate-x-10'}`}>
            <div className="flex items-center gap-3 mb-4">
                <Fingerprint className="text-emerald-500 w-6 h-6" />
                <span className="font-mono text-emerald-500 text-sm uppercase tracking-wider">Database_Record: USR_001</span>
            </div>
            <h2 className="text-3xl md:text-4xl font-bold text-white max-w-xl font-mono tracking-tight">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-12 items-center">
            ...
            {/* Bio & Data (Left Side) */}
            <div className={`lg:col-span-7 flex flex-col justify-center transition-all duration-1000 order-2 lg:order-1 ${isVisible ? 'opacity-100 translate-x-0' : 'opacity-0 -translate-x-10'}`}>
                
                {/* Data Grid */}
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-8">
                    <div className="p-4 bg-zinc-900/50 border border-zinc-800 rounded-sm flex items-center gap-4 hover:border-emerald-500/30 transition-colors group">
                        <div className="p-2 bg-black rounded text-emerald-500 group-hover:text-white transition-colors"><Terminal size={20} /></div>
                        <div>
                            <p className="text-[10px] text-zinc-500 uppercase font-mono">Codename</p>
                            <p className="text-white font-mono font-bold">DEV_PDR</p>
                        </div>
                    </div>
                    <div className="p-4 bg-zinc-900/50 border border-zinc-800 rounded-sm flex items-center gap-4 hover:border-emerald-500/30 transition-colors group">
                        <div className="p-2 bg-black rounded text-emerald-500 group-hover:text-white transition-colors"><Award size={20} /></div>
                        <div>
                            <p className="text-[10px] text-zinc-500 uppercase font-mono">XP Level</p>
                            <p className="text-white font-mono font-bold">17 Anos / Junior</p>
                        </div>
                    </div>
                </div>
                ...
                <div className="mb-8 relative">
                     <div className="absolute left-0 top-0 bottom-0 w-[2px] bg-gradient-to-b from-emerald-500 via-emerald-900 to-transparent"></div>
                     <h3 className="text-xl text-white font-bold font-mono mb-4 flex items-center gap-2 pl-6">
                        <span className="text-emerald-500">&gt;</span> System_Bio_Log
                     </h3>
                     <p className="text-zinc-400 leading-relaxed font-mono text-sm pl-6">
                        "Apesar de novo, opero com a maturidade de um sênior. Sou um desenvolvedor Full-Stack e especialista em Cibersegurança focado em entregar arquiteturas blindadas e código limpo. 
                        <br/><br/>
                        Estou no mercado há 1 ano, dominando mais de 10 linguagens e acumulando 14 projetos entregues com excelência. Meu diferencial é a velocidade aliada à segurança extrema, oferecendo soluções de nível corporativo a preços acessíveis que o mercado tradicional não consegue bater."
                     </p>
                </div>

                {/* Skills Matrix */}
                <div>
                    <h3 className="text-sm text-zinc-500 font-mono uppercase tracking-widest mb-4 flex items-center gap-2">
                        <Cpu size={14} /> Skill_Matrix_V1
                    </h3>
                    <div className="flex flex-wrap gap-2">
                        {skills.map((skill, index) => (
                            <span 
                                key={index}
                                className="px-3 py-1.5 bg-black border border-zinc-800 text-zinc-400 text-xs font-mono uppercase hover:text-emerald-400 hover:border-emerald-500/50 transition-all cursor-default hover:bg-emerald-500/5"
                            >
                                [{skill}]
                            </span>
                        ))}
                         <span className="px-3 py-1.5 bg-emerald-500/10 border border-emerald-500/30 text-emerald-400 text-xs font-mono uppercase animate-pulse">
                            + Learning_New_Tech...
                        </span>
                    </div>
                </div>
            ...
            {/* Map / Geographic Intelligence (Right Side) */}
            <div className={`lg:col-span-5 relative h-[500px] w-full bg-zinc-900/30 border border-zinc-800 rounded-sm overflow-hidden group order-1 lg:order-2 transition-all duration-1000 delay-200 ${isVisible ? 'opacity-100 translate-x-0' : 'opacity-0 translate-x-10'}`}>
                ...
                <div className="absolute inset-0 pointer-events-none z-20">
                    ...
                    {/* Top Info Bar */}
                    <div className="absolute top-4 left-8 right-8 h-8 flex justify-between items-center text-[10px] font-mono text-emerald-500/70 uppercase tracking-widest">
                        <div className="flex items-center gap-2">
                            <Wifi size={12} className="animate-pulse" />
                            <span>SAT_LINK: ONLINE</span>
                        </div>
                        <div className="flex items-center gap-2">
                            <span>BRAZIL OPS</span>
                        </div>
                    </div>

                    {/* Bottom Info Bar */}
                    <div className="absolute bottom-4 left-8 right-8 h-8 flex justify-between items-center text-[10px] font-mono text-emerald-500/70">
                         <div className="bg-black/60 px-2 py-1 border border-emerald-500/20 rounded flex gap-2">
                            <MapPin size={12} />
                            LAT: {coords.lat.toFixed(4)}
                         </div>
                         <div className="bg-black/60 px-2 py-1 border border-emerald-500/20 rounded">
                            LNG: {coords.lng.toFixed(4)}
                         </div>
                    </div>
                </div>

                {/* INLINE SVG MAP CONTAINER */}
                <div className="absolute inset-0 flex items-center justify-center z-10 p-8 bg-zinc-950">
                    <div className="relative w-full h-full flex items-center justify-center">
                        
                        {/* SVG Map of Brazil (Specific SVG provided by user, adapted for JSX) */}
                        <svg
                            version="1.1"
                            id="svg-map"
